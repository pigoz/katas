...
class TwoPairsRuleTest extends FunSuite with ShouldMatchers {
  ...
  test("Two Pairs scores 8 on 1,1,2,3,3") {
    yahtzeeRule = new TwoPairsRule
    yahtzeeRule.score(1,1,2,3,3) should be (8)
  }

  test("Two Pairs scores 8 on 2,2,2,3,3") {
    yahtzeeRule = new TwoPairsRule
    yahtzeeRule.score(2,2,2,3,3) should be (10)
  }
}
